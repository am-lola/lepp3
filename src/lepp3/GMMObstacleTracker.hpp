#ifndef LEPP3_GMM_OBSTACLE_TRACKER_H__
 ...
 */
class GMMObstacleTracker : public FrameDataObserver, public FrameDataSubject
{
public:

  GMMObstacleTracker(GMM::ObstacleTrackerParams const& parameters)
...
private:
  GMM::ObstacleTrackerParams _parameters;
  /**
  ...
  /**
   * Notifies any `aggregators_` that wishes to receive obstacle tracking data.
   */
  ...
  void generateSSVs(GMM::State& state, FrameDataPtr frameData) const;
...
static float logpdf(const GMM::State& state, const Eigen::Vector4f& point)
{
...
static float pdf(const GMM::State& state, const Eigen::Vector4f& point)
{
...
void GMMObstacleTracker::initialize(const PointCloudT* pc)
{
  Eigen::Vector3f mean;
...
void GMMObstacleTracker::emstep(const PointCloudT* pc, int frameNum)
{
  ...
  Vector<GMM::State> newStates; // states that should be added
...
void GMMObstacleTracker::fitSplittingGaussian(const PointCloudT* pc, const Eigen::MatrixXf& R, int state,
                                           int vclusterA, int vclusterB, Eigen::Vector4f& outMeanA, Eigen::Vector4f& outMeanB,
                                           Eigen::Matrix4f& outCovA, Eigen::Matrix4f& outCovB) const
{
  outCovA.setZero();
...
void GMMObstacleTracker::fitSSVs(int k, const PointCloudT* pc, const Eigen::MatrixXf& R, const Eigen::MatrixXi& C)
{
  using namespace Eigen;

  GMM::State& state = _states[k];
  GMM::ObstacleSSVData& ssvData = state.ssvData;
...
void GMMObstacleTracker::generateSSVs(GMM::State& state, FrameDataPtr frameData) const
{
  using namespace Eigen;
...
void GMMObstacleTracker::addState(GMM::State& state)
{
  notifyTrackerData(state, GMM::VisModifyFlag::INIT);
  _states.push_back(state);
}

void GMMObstacleTracker::addState(const Eigen::Vector3f& mean, const Eigen::Matrix3f& covar)
{
  _states.emplace_back(mean, covar);
  notifyTrackerData(_states.back(), GMM::VisModifyFlag::INIT);
}

void GMMObstacleTracker::removeState(size_t index)
{
  notifyTrackerData(_states[index], GMM::VisModifyFlag::DE_INIT);
...
void GMMObstacleTracker::clearStates()
{
  for (size_t k = 0; k < _states.size(); k++)
    notifyTrackerData(_states[k], GMM::VisModifyFlag::DE_INIT);

  _states.clear();
}


void GMMObstacleTracker::updateFrame(FrameDataPtr frameData)
{
  using namespace Eigen;
  ...
  for (auto& state : _states)
  {
    ...
    // the kalman filter).
    notifyTrackerData(state, GMM::VisModifyFlag::UPDATE);
  }

  ar::PointCloudData custom_color_cloud_data(ar::PCL_PointXYZRGBA);
  ...
  custom_color_cloud_data.pointData = reinterpret_cast<const void*>(_visualizerPointCloud.data());
  custom_color_cloud_data.numPoints = pc->size();
  runtime_stats.MainAlgorithmTime = perfTimer.duration();
  // Notify any attached observer of the point cloud, voxel grid and
  // algorithm's runtime information.
  notifyTrackerData(custom_color_cloud_data, _voxelGrid, runtime_stats);
...
void GMMObstacleTracker::notifyTrackerData(
  ...
  size_t sz = aggregators_.size();
  for (size_t i = 0; i < sz; ++i) {
    aggregators_[i]->updateObstacleState(state, flag);
  }

}

// TODO: [Sahand] Decide whether to remove this method
//       I think there's no need to send an extra customized point cloud to
//       listeners.
...
#endif // LEPP3_GMM_OBSTACLE_TRACKER_H__
